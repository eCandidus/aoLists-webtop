...
exports.send = function (req, res, credentials, values) {
    aofn.serverCall(credentials, aofn.format('/users/{0}/{1}/validate', values.userNickname, values.password), function (response) {
            ...
            aofn.serverCall(values.userNickname + ':' + values.password, '/metadata', function (response) {
                ...
                sets.forEach(function (element, index, array) {
                    ...
                    if (set.length > 1) {
                        ...
                        aofn.serverCall(credentials, aofn.format('/{0}/{1}/metadata', db, collection), function (lresp) {
                            // Valid?
                            if (lresp._desc) {
                                // The top level item
